...
class HotelManager(object):
    ...
    def add_room(self, hotel_id, number, type, max_guests, description, amenities, price):
        ...
        room = Room(hotel_id=hotel_id, number=number, type=type, max_guests=max_guests, description=description, amenities=amenities, price=price)
        session.add(room)
        session.commit()
    ...
    def update_hotel(self, hotel_id: int, street=None, zip_code=None, city=None, **kwargs):
        session = self.get_session()
        """updates all info of a hotel that is provided by the user in the console"""
        try:
            hotel = session.query(Hotel).filter(Hotel.id == hotel_id).one()
    ...
    def change_room_availability(self, hotel_id: int, room_number: str,
                                     unavailability_start: date, unavailability_end: date):
        """
        Change the availability of a room. By setting a timeframe (start and end-date) in which a room is not available.
        It then checks whether rooms are arleady booked in that timeframe and deletes them if the user confirms.
        ...
        if active_bookings:
            ...
            confirmation = input("Do you want to proceed with deleting the bookings and changing the availability? (yes/no): ")
        ...
        try:
            ...
            session.rollback()
            return "Failed to update availability. Error: " + str(e)
        return f"Room {room_number} availability changed to UA start{unavailability_start} and UA end{unavailability_end}."
